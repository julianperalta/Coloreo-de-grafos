...
public class GrafoNDNP {
	...
	public GrafoNDNP(String path) throws Exception {
		int indice;
		...
		for (int i = 0; i < this.cantAristas; i++) {
			...
			this.grafo.setFilaColumna(fil, col, true);
		}
	...
	private void colorear() throws Exception {
		int color = 1;
		int nodo;
		int j;
//		int indice;
		...
		for (int i = 1; i < this.cantNodos; i++) {
			...
			while (j < this.cantNodos) {
				if (nodo != j) {
//					if (nodo < j)
//						indice = this.grafo.getI(nodo, j);
//					else
//						indice = this.grafo.getI(j, nodo);

					if (this.grafo.getFilaColumna(nodo, j) && this.nodosColoreados[nodo] == this.nodosColoreados[j]) {
						color++;
						if (color > colorMax)
							colorMax = color;
						this.nodosColoreados[nodo] = color;
	...
	public void coloreoSecuencial(int corridas) throws Exception {
		int nroCorrida = 0;
	...
	public int coloreoWelshPowell(int corridas) throws Exception {
		int nroCorrida = 0;
		for (int i = 0; i < corridas; i++) {
			Collections.shuffle(this.nodos);
		...
		return this.mejorColor;
	}

	public int coloreoMatula(int corridas) throws Exception {
		int nroCorrida = 0;
		for (int i = 0; i < corridas; i++) {
			Collections.shuffle(this.nodos);
			Collections.sort(this.nodos, new Comparator<Nodo>() {
	...
	private void escribirSolucion(String algoritmo) throws IOException {
		FileWriter file = new FileWriter("./Resultados/COLOREO" + "_" + algoritmo + "_" + this.cantNodos + "_"
				+ this.ptajeAdyacencia + ".out");
		...
		buffer.write(" ");
		buffer.write(String.valueOf(this.ptajeAdyacencia));
		buffer.write(" ");
	...
	private void escribirVectorEstadistico(String algoritmo) throws IOException {
		FileWriter file = new FileWriter("./Estadisticas/VECTOR" + "_" + algoritmo + "_" + this.cantNodos + "_"
				+  this.ptajeAdyacencia + ".out");
		BufferedWriter buffer = new BufferedWriter(file);
